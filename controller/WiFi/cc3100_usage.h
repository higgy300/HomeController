...
/* Juanito:
 * The User fuctions below are the only ones you need.
 * File to modify:
 * sl_common.h -> change SSID_NAME and PASSKEY macros.
 * This should work if both devices do not have an static IP. Meaning that 
 * the IP is assigned by the router.
 *
 * This an example of their usage:
 * spi_Open(0,0);
 * initCC3100(Client);
 * SendData(car_info_ptr,CTRL_IP_ADDR,sizeof(car_info));
 * ReceiveData(car_info_ptr,sizeof(car_info));
 */
 ...
 */
#define HOST_IP_ADDR           SL_IPV4_VAL(192,168,1,3)               // IP address of server to connect to
